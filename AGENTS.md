...
- **Asset canvas:** browse all images + copy captions via `gallery/` — from repo root run `python3 -m http.server 8765` and open `http://localhost:8765/gallery/`. Caption map is `gallery/captions.json`; refresh file list with a quick inventory rebuild if many new images were added (see `gallery/inventory.json`).
